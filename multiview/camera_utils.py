# reading and writing camera parameters
import cv2
import numpy as np
import os
from utils.utils import point_to_line
...
def write_extri_single(extri_name, cameras):
...
def read_intri(intri_name, cam_names=[]):
    assert os.path.exists(intri_name), intri_name
...
def get_fundamental_matrix(intri_1,RT_1, intri_2, RT_2):
    fundamental_op = lambda K_1, R_1, T_1, K_0, R_0, T_0: np.linalg.inv(K_0).T @ (
    ...
    fundamental_RT_op = lambda K_0, RT_0, K_1, RT_1: fundamental_op (K_0, RT_0[:, :3], RT_0[:, 3:], K_1,
                                                                          RT_1[:, :3], RT_1[:, 3:] )
    return fundamental_RT_op(intri_1, RT_1, intri_2, RT_2)


def get_fundamental_matrix_all(cameras, basenames):
    ...
    F = {(icam, jcam): np.zeros((3, 3)) for jcam in basenames for icam in basenames if icam != jcam}
...
def compute_epipolar_error(pts1, pts2, F):
    ...
    lines1, lines2 = compute_epipolar_lines(pts1, pts2, F)
    error1 = point_to_line(pts1,lines1)
    error2 = point_to_line(pts2,lines2)
